...
class TestValidRegistrationRT:
    ...
    def test_clicable_navi_bar(self):
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.XPATH, RTPanelNaviBar.LOCATOR_NAVI_BAR_LS))).click()
        assert self.driver.find_element(By.XPATH, RTPanelNaviBar.LOCATOR_FORM_LS)
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.XPATH, RTPanelNaviBar.LOCATOR_NAVI_BAR_MAIL))).click()
        assert self.driver.find_element(By.XPATH, RTPanelNaviBar.LOCATOR_FORM_MAIL)
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.XPATH, RTPanelNaviBar.LOCATOR_NAVI_BAR_LOGIN))).click()
        assert self.driver.find_element(By.XPATH, RTPanelNaviBar.LOCATOR_FORM_LOGIN)
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.XPATH, RTPanelNaviBar.LOCATOR_FORM_TELEPHONE))).click()
...
# 19
    @allure.feature('Авторизация по номеру телефона и паролю, положительный тест')
    def test_auth_page_phone_login_valid(self):
        self.driver.find_element(By.ID, RTAutorizationLocators.LOCATOR_RT_AUTORIZATION_USER).send_keys(Valid_Data.valid_phoneNumber)
        self.driver.find_element(By.ID, RTAutorizationLocators.LOCATOR_RT_AUTORIZATION_PASSWORD).send_keys(Valid_Data.valid_password)
        self.driver.find_element(By.ID, RTAutorizationLocators.LOCATOR_RT_AUTORIZATION_BUTTON_LOGIN).click()
# 20
    @allure.feature('Авторизация по email и паролю, положительный тест')
    def test_auth_page_phone_login_valid(self):
        self.driver.find_element(By.ID, RTAutorizationLocators.LOCATOR_RT_AUTORIZATION_USER).send_keys(Valid_Data.valid_email)
        self.driver.find_element(By.ID, RTAutorizationLocators.LOCATOR_RT_AUTORIZATION_PASSWORD).send_keys(Valid_Data.valid_password)
        self.driver.find_element(By.ID, RTAutorizationLocators.LOCATOR_RT_AUTORIZATION_BUTTON_LOGIN).click()